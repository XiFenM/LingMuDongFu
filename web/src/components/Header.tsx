...
import { NAV, SITE, SOCIAL } from "@/lib/data";
      ...
      <nav className="mx-auto flex max-w-[1280px] items-center gap-8 px-6 py-3 lg:py-4">
        <Link href="/" className="group flex items-center gap-2.5">
          <span className="relative inline-flex h-10 w-10 overflow-hidden rounded-full ring-1 ring-[rgba(192,163,93,0.38)]">
            <Image src="/assets/lydt/logo/logo-lydt-mark.png" alt="LYDT" fill sizes="40px" className="object-cover transition group-hover:scale-105" />
          </span>
        ...
        <div className="ml-auto flex items-center gap-3 text-[#cfc4a8]">
          <a href={SOCIAL.github} target="_blank" rel="noreferrer" className="rounded-md p-2 transition hover:bg-[rgba(156,240,189,0.08)] hover:text-[var(--color-firefly-300)]" aria-label="GitHub">
            <IconGithub />
          </a>
          <a href={SOCIAL.linkedin} target="_blank" rel="noreferrer" className="rounded-md p-2 transition hover:bg-[rgba(156,240,189,0.08)] hover:text-[var(--color-firefly-300)]" aria-label="LinkedIn">
            <IconLinkedIn />
          </a>
